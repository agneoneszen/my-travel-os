...
function AddExpenseModal({ tripId, categories, members, initialData, onClose, onSave }) {
    const [formData, setFormData] = useState(initialData || { 
        ...
        location: '', notes: '', payer: members[0] || '我', forWho: '全體', 
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-end sm:items-center justify-center" onClick={onClose}>
        <div onClick={e => e.stopPropagation()} className="bg-white w-full sm:max-w-lg rounded-t-[2rem] p-8 shadow-2xl animate-slide-up max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
             ...
             <div className="grid grid-cols-2 gap-4">
                 ...
                 <div>
                    ...
                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                        {categories.map(c => (
                            <button key={c} type="button" onClick={() => setFormData({...formData, category: c})} className={`flex-shrink-0 px-3 py-1 rounded-lg text-xs font-bold border transition-colors ${formData.category === c ? 'bg-hero-sky-500 text-white border-hero-sky-500' : 'bg-white text-hero-dark-muted border-hero-sand-200 hover:border-hero-sky-300'}`}>{c}</button>
                        ))}
                    </div>
                 </div>
             </div>
             ...
             <div>
                <label className="text-[10px] font-bold text-hero-dark-muted uppercase mb-1 block">收據 / 照片</label>
                <div className="flex gap-4 items-center">
                    {formData.image && (
                        <div className="relative w-20 h-20 rounded-xl overflow-hidden shadow-sm border border-hero-sand-200 group">
                            <img src={formData.image} alt="Receipt" className="w-full h-full object-cover" />
                            <button type="button" onClick={() => setFormData(prev => ({...prev, image: ''}))} className="absolute inset-0 bg-black/40 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16}/></button>
                        </div>
                    )}
                    <label className="flex-1 cursor-pointer">
                        <div className="flex items-center justify-center gap-2 w-full p-4 bg-hero-sand-50 border-2 border-dashed border-hero-sand-200 rounded-xl text-hero-dark-muted hover:border-hero-dark-muted hover:text-hero-dark transition-all">
                            <Camera size={20} />
                            <span className="text-xs font-bold">上傳收據</span>
                        </div>
            ...
            {expenses.map((item) => (
                <div key={item.id} onClick={() => handleOpenEdit(item)} className="bg-white p-4 rounded-2xl border border-hero-sand-200 flex justify-between items-center group cursor-pointer hover:border-hero-sky-300 transition-all hover:shadow-md">
                    ...
                    <div className="flex items-center gap-3">
                        {item.image && (
                            <img src={item.image} alt="Receipt" className="w-10 h-10 rounded-lg object-cover border border-hero-sand-200" />
                        )}
